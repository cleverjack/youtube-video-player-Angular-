angular.module('app').controller('ContextMenuController', function($rootScope, $scope, $stateParams, $translate, playlists, contextMenu, modal, player, userLibrary, clipboard, users, utils) {
    ...
    $scope.copyAlbumLink = function() {
        ...
        clipboard.copy($rootScope.baseUrl+(! utils.getSetting('enablePushState') ? '#/' : '')+'album/'+artist.name+'/'+utils.encodeUrlParam(contextMenu.item.name));
    };

    $scope.copyArtistLink = function() {
        clipboard.copy($rootScope.baseUrl+(! utils.getSetting('enablePushState') ? '#/' : '')+'artist/'+utils.encodeUrlParam(contextMenu.item.name));
    };
